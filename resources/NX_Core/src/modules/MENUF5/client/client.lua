...
function UpDev.OpenPersonalMenu()
    ...
    local devmenu = RageUI.CreateSubMenu(menuPrincipal, "Dev Menu", "Menu développeur")
    menuPrincipal.Closed = function()
        openmenu = false
    end
    ...
    if openmenu then
        ...
        CreateThread(function()
            while openmenu do
                ...
                RageUI.Button("Préférences", "Définissez vos préférences", {RightLabel = "→"}, true, {}, visualOption)
                RageUI.Button("Guide", nil, {RightLabel = "→"}, true, {}, keyPadInfo)
                RageUI.Button("Menu développeur", "Ouvrez le menu développeur mais ATTENTION ce menu est disponible à tous !", {RightLabel = "→"}, true, {}, devmenu)
                end)
                ...
                RageUI.IsVisible(devmenu, function()
                    RageUI.Button("Activer/Désactiver Freecam", "Freecam auto directionnel Z = avancé, S = reculer, SHIFT = vitesse, D = droite, Q = gauche", {RightLabel = noclip and "Actif" or "Inactif"} , true, {
                        onSelected = function()
                            noclip = not noclip
                            if noclip then 
                                ToggleNoClipMode()
                                ESX.ShowNotification("Freecam activé !")
                            else
                                ToggleNoClipMode()
                                ESX.ShowNotification("Freecam désactivé !")
                            end
                        end
                    })
                    RageUI.Button("TP Marker", "Vous pouvez vous TP sur un marker sur la map", {RightLabel = "→"}, true, {
                        onSelected = function()
                            ...
                            if DoesBlipExist(waypoint) then
                                ...
                                ESX.ShowNotification("Téléporté au marker !")
                            else
                                ESX.ShowNotification("Aucun marker n'est placé sur la carte.")
                            end
                        end
                    })  
                    RageUI.Line()
                    RageUI.Button("Copier Vector3", "Copie tes coordonnées au format vector3", {RightLabel = "→"}, true, {
                        onSelected = function()
                            local coords = GetEntityCoords(PlayerPedId())
                            local vector = string.format("vector3(%.2f, %.2f, %.2f)", coords.x, coords.y, coords.z)
                            SendNUIMessage({ type = "clipboard", text = vector })
                            ESX.ShowNotification("Vector3 copié !")
                        end
                    })

                    RageUI.Button("Copier Vector4", "Copie tes coordonnées au format vector4", {RightLabel = "→"}, true, {
                        onSelected = function()
                            local coords = GetEntityCoords(PlayerPedId())
                            local heading = GetEntityHeading(PlayerPedId())
                            local vector = string.format("vector4(%.2f, %.2f, %.2f, %.2f)", coords.x, coords.y, coords.z, heading)
                            SendNUIMessage({ type = "clipboard", text = vector })
                            ESX.ShowNotification("Vector4 copié !")
                        end
                    })

                    RageUI.Button("Copier X Y Z", "Copie tes coordonnées sous forme X Y Z", {RightLabel = "→"}, true, {
                        onSelected = function()
                            local coords = GetEntityCoords(PlayerPedId())
                            local vector = string.format("%.2f, %.2f, %.2f", coords.x, coords.y, coords.z)
                            SendNUIMessage({ type = "clipboard", text = vector })
                            ESX.ShowNotification("X Y Z copié !")
                        end
                    })

                    RageUI.Button("Copier {x=, y=, z=}", "Copie tes coordonnées sous forme de table Lua", {RightLabel = "→"}, true, {
                        onSelected = function()
                            local coords = GetEntityCoords(PlayerPedId())
                            local vector = string.format("{x = %.2f, y = %.2f, z = %.2f}", coords.x, coords.y, coords.z)
                            SendNUIMessage({ type = "clipboard", text = vector })
                            ESX.ShowNotification("{x=, y=, z=} copié !")
                        end
                    })
                    RageUI.Button("Copier Heading", "Copie ton heading", {RightLabel = "→"}, true, {
                        onSelected = function()
                            local heading = GetEntityHeading(PlayerPedId())
                            SendNUIMessage({ type = "clipboard", text = string.format("%.2f", heading) })
                            ESX.ShowNotification("Heading copié !")
                        end
                    })
                end)
