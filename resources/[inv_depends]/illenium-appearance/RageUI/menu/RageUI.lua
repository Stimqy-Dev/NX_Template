...
function RageUI.SetScaleformParams(scaleform, data)
    data = data or {}
    for i = 1, #data do
        local v = data[i]  -- Accéder à l'élément de data avec l'index i
        ...
        if CurrentMenu() and (CurrentMenu.Display.Header) then
                ...
                if CurrentMenu.Sprite.Dictionary ~= nil then
                    if CurrentMenu.Sprite.Dictionary == "commonmenu" then
                        RenderSprite(CurrentMenu.Sprite.Dictionary, CurrentMenu.Sprite.Texture, CurrentMenu.X,
                            ...
                            CurrentMenu.Sprite.Color.G, CurrentMenu.Sprite.Color.B, 216)
                    else
                        RenderSprite(CurrentMenu.Sprite.Dictionary, CurrentMenu.Sprite.Texture, CurrentMenu.X,
                            CurrentMenu.Y + 10, RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset,
                            RageUI.Settings.Items.Title.Background.Height - 10, nil)
                    end
                else
            ...
            if (CurrentMenu.Display.Glare) then
                ...
                local GlareX = CurrentMenu.X / 1920 +
                    (CurrentMenu.SafeZoneSize.X / (67 - (CurrentMenu.WidthOffset * 0.065731)))
                ---@type number
                local GlareY = CurrentMenu.Y / 1080 + CurrentMenu.SafeZoneSize.Y / 33.900
        ...
        if CurrentMenu() and (CurrentMenu.Display.Subtitle) then
            ...
            if CurrentMenu.Subtitle ~= "" then
                RenderRectangle(CurrentMenu.X, CurrentMenu.Y + RageUI.ItemOffset,
                    ...
                    RageUI.Settings.Items.Subtitle.Background.Height + CurrentMenu.SubtitleHeight - 3, 22, 22, 22, 150)
                ...
                if CurrentMenu.Display.PageCounter then
                    if CurrentMenu.PageCounter == nil then
                        RenderText(CurrentMenu.PageCounterColour .. CurrentMenu.Index .. " / " .. CurrentMenu.Options,
                            ...
                            CurrentMenu.Y + RageUI.Settings.Items.Subtitle.PreText.Y + RageUI.ItemOffset, 8,
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Background) then
            ...
            RenderRectangle(CurrentMenu.X,
                146 + CurrentMenu.Y + RageUI.Settings.Items.Background.Y + CurrentMenu.SubtitleHeight,
                RageUI.Settings.Items.Background.Width + CurrentMenu.WidthOffset, RageUI.ItemOffset - 117, 22, 22, 22,
                150)
            RenderRectangle(CurrentMenu.X,
                CurrentMenu.Y + RageUI.Settings.Items.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 27,
                RageUI.Settings.Items.Background.Width + CurrentMenu.WidthOffset, 2, 0, 0, 0, 0)
...
function RageUI.Description()
    ...
    if CurrentMenu ~= nil and CurrentMenu.Description ~= nil then
        if CurrentMenu() then
            ...
            -- RenderRectangle(CurrentMenu.X,
            --     CurrentMenu.Y + Description.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 28,
            --     Description.Background.Width + CurrentMenu.WidthOffset, CurrentMenu.DescriptionHeight + 5, 16, 16, 16,
            --     125)

            RenderSprite("commonmenu", "backgroundpanel", CurrentMenu.X - 17,
                CurrentMenu.Y + Description.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 28,
                Description.Background.Width + CurrentMenu.WidthOffset + 38, CurrentMenu.DescriptionHeight + 5, 0, 16, 16,
                16,
                125)

            RenderText(CurrentMenu.Description, CurrentMenu.X + Description.Text.X,
                CurrentMenu.Y + Description.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 30, 0,
...
function RageUI.ItemsDescription(CurrentMenu, Description, Selected)
    ...
    if Description ~= "" or Description ~= nil then
        ...
        if Selected and CurrentMenu.Description ~= Description then
            ...
            ---@type number
            local DescriptionLineCount = GetLineCount(CurrentMenu.Description, CurrentMenu.X + SettingsDescription.Text
                .X, CurrentMenu.Y + SettingsDescription.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0,
                SettingsDescription.Text.Scale, 20, 24, 28, 1, nil, false, false,
