...
function client.openInventory(inv, data)
	...
	CreateThread(function()
		exports['pedscreen']:PedScreenCreate({
			dict = "anim@amb@nightclub@peds@", 
			anim = "rcmme_amanda1_stand_loop_cop"
		})  
	end)
	Utils.PlayAnim(0, shared.animDict, shared.animName, 5.0, 1.5, -1, 48, 0.0, 0, 0, 0)
    return true
end
...
lib.callback.register('ox_inventory:usingItem', function(data, noAnim)
	...
	if item and usingItem then
		...
		-- 		animationDictionary = item.anim.dict,
		-- 		animationName = item.anim.name
		-- 	},
		-- 	onComplete = function()
...
local function registerCommands()
	...
	for i = 1, 5 do
		lib.addKeybind({
			...
			onPressed = function()
				if invOpen or EnableWeaponWheel or not invHotkeys or IsNuiFocused() then useSlot(i) end
			end
		})
	end
...
function client.closeInventory(server)
	...
	if invOpen then
		...
		closeTrunk()
		CreateThread(function()
			exports['pedscreen']:PedScreenDelete()
		end)
		StopAnimTask(PlayerPedId(), shared.animDict, shared.animName, 1.0)
...
RegisterNUICallback('is_not_cloth', function(data,cb)
    lib.notify({ type = 'error', description = 'Vous ne pouvez pas mettre cette item sur cette emplacements !' })
    cb(true)
end)

RegisterNUICallback('is_not_cloth_swap', function(data,cb)
    lib.notify({ type = 'error', description = "Vous ne pouvez pas échangez d'emplacements ces deux items !"})
    cb(true)
end)

RegisterNUICallback('SearchDisableInput', function(data, cb)
	SetNuiFocusKeepInput(not data.status)
	cb(true)
end)

RegisterNUICallback('previewCloth', function(data, cb)
	TriggerEvent('ox_inventory:previewCloth', data)
	cb(true)
end)

RegisterNUICallback('stopPreviewCloth', function(data, cb)
	TriggerEvent('ox_inventory:stopPreviewCloth', data)
	cb(true)
end)
RegisterNetEvent('ox_inventory:SendNotifIsNotCloth', function()
    lib.notify({ type = 'error', description = 'Vous ne pouvez pas mettre cette item sur cette emplacements !' })
end)

RegisterNUICallback('renameItem', function(slot, cb)
    SetNuiFocusKeepInput(false)
    local label = lib.inputDialog("Renommer L'item", {"Nom de l'item"})
    result = lib.callback.await('ox_inventory:renameItem', 200, slot, label[1])
    SetNuiFocusKeepInput(true)
    cb(true)
end)