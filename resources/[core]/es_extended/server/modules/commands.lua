ESX.RegisterCommand({'id', 'identifiants', 'uniqueid', 'uid', 'discordid', 'did'}, 'user', function(xPlayer)
	if not xPlayer then
		return showError('[^1ERROR^7] The xPlayer value is nil')
	end
	xPlayer.showNotification('Vos id\'s sont:\nUnique: '.. xPlayer.unique_id ..'\nTemporaire: '.. xPlayer.source ..'\nDiscord: '.. GetPlayerIdentifierByType(xPlayer.source, 'discord'):match("(%d+)"))
end, false, {
	help = 'Connaître ses identifiants',
	validate = false,
})
...
ESX.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args, showError)
        if not ESX.DoesCrewExist(args.crew, args.grade) then
            return showError(TranslateCap("command_setcrew_invalid"))
        end

        args.playerId.setCrew(args.crew, args.grade)
        if Config.AdminLogging then
            ESX.DiscordLogFields("UserActions", "Set Crew /setcrew Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
    ...
    true,
    {
        help = TranslateCap("command_setcrew"),
        validate = true,
        arguments = {
            { name = "playerId", help = TranslateCap("commandgeneric_playerid"), type = "player" },
            { name = "crew", help = TranslateCap("command_setcrew_crew"), type = "string" },
            { name = "grade", help = TranslateCap("command_setcrew_grade"), type = "number" },
        },
    }
)
...
ESX.RegisterCommand("refreshcrew", "admin", function()
    ESX.RefreshCrews()
end, true, { help = TranslateCap("command_clearall") })
...
ESX.RegisterCommand("crew", { "user", "admin" }, function(xPlayer, _, _)
	local crew = xPlayer.getCrew()

    print(("%s, your crew is: ^5%s^0 - ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), crew.name, crew.grade_label, crew.onDuty and "On Duty" or "Off Duty"))
end, false)

ESX.RegisterCommand("info", { "user", "admin" }, function(xPlayer)
    local job = xPlayer.getJob().name
    local crew = xPlayer.getCrew().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0 | ^2Crew: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job, crew))
end, false)
