...
function CreateExtendedPlayer(playerId, identifier, unique_id, group, accounts, inventory, weight, job, crew, loadout, name, coords, metadata)
    ---@class xPlayer
    local self = {}

    self.unique_id = tonumber(unique_id)
    ...
    if type(self.metadata.crewDuty) ~= "boolean" then
        self.metadata.crewDuty = self.crew.name ~= "nocrew" and Config.DefaultCrewDuty or false
    end
    crew.onDuty = self.metadata.crewDuty

    ExecuteCommand(("add_principal identifier.%s group.%s"):format(self.license, self.group))

    local stateBag = Player(self.source).state
    stateBag:set("identifier", self.identifier, false)
	stateBag:set("unique_id", self.unique_id, true)
    stateBag:set("license", self.license, false)
    stateBag:set("job", self.job, true)
    stateBag:set("crew", self.crew, true)
    ...
    ---@param newCrew string
    ---@param grade string
    ---@param onDuty? boolean
    ---@return nil
    function self.setCrew(newCrew, grade, onDuty)
        grade = tostring(grade)
        local lastCrew = self.crew

        if not ESX.DoesCrewExist(newCrew, grade) then
            return print(("[ESX] [^3WARNING^7] Ignoring invalid ^5.setCrew()^7 usage for ID: ^5%s^7, Crew: ^5%s^7"):format(self.source, newCrew))
        end

        if newCrew == "nocrew" then
            onDuty = false
        end

        if type(onDuty) ~= "boolean" then
            onDuty = Config.DefaultCrewDuty
        end

        local crewObject, gradeObject = ESX.Crews[newCrew], ESX.Crews[newCrew].grades[grade]
        ...
        self.metadata.crewDuty = onDuty
        TriggerEvent("esx:setCrew", self.source, self.crew, lastCrew)
        self.triggerEvent("esx:setCrew", self.crew, lastCrew)
        Player(self.source).state:set("crew", self.crew, true)
    end
