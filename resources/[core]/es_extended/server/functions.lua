                    ...
                    for k, v in ipairs(command.suggestion.arguments) do
                        if v.type then
                            ...
                            elseif v.type == "player" or v.type == "playerId" then
                                ...
                                if args[k] == "me" then
                                    targetPlayer = ESX.GetPlayerFromId(playerId).unique_id
                                end

                                if targetPlayer then
                                    local xTargetPlayer = ESX.GetPlayerFromUniqueId(targetPlayer)
...
function Core.SavePlayer(xPlayer, cb)
    ...
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `crew` = ?, `crew_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ? WHERE `identifier` = ?",
        parameters,
...
local function checkTable(key, val, xPlayer, xPlayers)
    for valIndex = 1, #val do
        ...
        if (key == "job" and xPlayer.job.name == value) or (key == "crew" and xPlayer.crew.name == value) or xPlayer[key] == value then
...
function ESX.GetExtendedPlayers(key, val)
    ...
    for _, xPlayer in pairs(ESX.Players) do
        if (key == "job" and xPlayer.job.name == val) or (key == "crew" and xPlayer.crew.name == val) or xPlayer[key] == val then
...
function ESX.GetNumPlayers(key, val)
    ...
    if type(val) == "table" then
        ...
        if key == "crew" then
            for _, v in ipairs(val) do
                numPlayers[v] = (Core.CrewsPlayerCount[v] or 0)
            end
            return numPlayers
        end
    ...
    if key == "crew" then
        return (Core.CrewsPlayerCount[val] or 0)
    end
...
---@return table
function ESX.GetPlayerFromUniqueId(unique_id)
	if type(unique_id) == "string" then
		unique_id = tonumber(unique_id)
	end
	for _, xPlayer in pairs(ESX.Players) do
		if xPlayer.unique_id == unique_id then
			return xPlayer
		end
	end
	return nil
...
---@return nil
function ESX.RefreshCrews()
    local Crews = {}
    local crews = MySQL.query.await("SELECT * FROM crews")

    for _, v in ipairs(crews) do
        Crews[v.name] = v
        Crews[v.name].grades = {}
    end
    ...
    for _, v in ipairs(crewGrades) do
        if Crews[v.crew_name] then
            Crews[v.crew_name].grades[tostring(v.grade)] = v
        else
            print(('[^3WARNING^7] Ignoring crew grades for ^5"%s"^0 due to missing crew'):format(v.crew_name))
        end
    end
    ...
    for _, v in pairs(Crews) do
        if ESX.Table.SizeOf(v.grades) == 0 then
            Crews[v.name] = nil
            print(('[^3WARNING^7] Ignoring crew ^5"%s"^0 due to no crew grades found'):format(v.name))
        end
    end

    if not Crews then
        -- Fallback data, if no crews exist
        ESX.Crews["nocrew"] = { label = "NoCrew", grades = { ["0"] = { grade = 0, label = "NoCrew", salary = 0, skin_male = {}, skin_female = {} } } }
    else
        ESX.Crews = Crews
    end
end
...
---@param grade string
---@return boolean
function ESX.DoesCrewExist(crew, grade)
    return (ESX.Crews[crew] and ESX.Crews[crew].grades[tostring(grade)] ~= nil) or false
end
