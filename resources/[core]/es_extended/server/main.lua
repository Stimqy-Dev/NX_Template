...
local loadPlayer = "SELECT `unique_id`, `accounts`, `job`, `job_grade`, `crew`, `crew_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`"
...
if Config.Multichar then
    AddEventHandler("esx:onPlayerJoined", function(src, char, data)
        while not next(ESX.Jobs) and not next(ESX.Crews) do
            Wait(50)
        end
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	local unique_id = tonumber(result.unique_id)
    ...
    -- crew
    local crew, grade = result.crew, tostring(result.crew_grade)

    if not ESX.DoesCrewExist(crew, grade) then
        print(("[^3WARNING^7] Ignoring invalid crew for ^5%s^7 [crew: ^5%s^7, grade: ^5%s^7]"):format(identifier, crew, grade))
        crew, grade = "nocrew", "0"
    end

    local crewObject, gradeObject = ESX.Crews[crew], ESX.Crews[crew].grades[grade]

    userData.crew = {
        id = crewObject.id,
        name = crewObject.name,
        label = crewObject.label,
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.unique_id, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.crew, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata)
...
AddEventHandler("playerDropped", function(reason)
    ...
    if xPlayer then
        ...
        local crew = xPlayer.getCrew().name
        local currentJob = Core.JobsPlayerCount[job]
        local currentCrew = Core.CrewsPlayerCount[crew]
        Core.JobsPlayerCount[job] = ((currentJob and currentJob > 0) and currentJob or 1) - 1
        Core.CrewsPlayerCount[crew] = ((currentCrew and currentCrew > 0) and currentCrew or 1) - 1

        GlobalState[("%s:count"):format(job)] = Core.JobsPlayerCount[job]
        GlobalState[("%s:count"):format(crew)] = Core.CrewsPlayerCount[crew]
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer)
    ...
    local crewKey = ("%s:count"):format(crew)

    Core.JobsPlayerCount[job] = (Core.JobsPlayerCount[job] or 0) + 1
    Core.CrewsPlayerCount[crew] = (Core.CrewsPlayerCount[crew] or 0) + 1
    GlobalState[jobKey] = Core.JobsPlayerCount[job]
    GlobalState[crewKey] = Core.CrewsPlayerCount[crew]
end)
...
AddEventHandler("esx:setCrew", function(_, crew, lastCrew)
    local lastCrewKey = ("%s:count"):format(lastCrew.name)
    local crewKey = ("%s:count"):format(crew.name)
    local currentLastCrew = Core.CrewsPlayerCount[lastCrew.name]

    Core.CrewsPlayerCount[lastCrew.name] = ((currentLastCrew and currentLastCrew > 0) and currentLastCrew or 1) - 1
    Core.CrewsPlayerCount[crew.name] = (Core.CrewsPlayerCount[crew.name] or 0) + 1

    GlobalState[lastCrewKey] = Core.CrewsPlayerCount[lastCrew.name]
    GlobalState[crewKey] = Core.CrewsPlayerCount[crew.name]
end)
